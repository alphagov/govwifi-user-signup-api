  ...
  describe "#get_matching" do
    context "when there are no records" do
      it "returns no results" do
        expect(subject.get_matching(number:, message:, within_minutes: 60).all).to be_empty
      end
    end

    context "when there are records" do
      it "returns matching messages with number and message" do
        described_class.insert(number:, message:)
        described_class.insert(number: "foo", message: "bar")
        described_class.insert(number:, message:)
        described_class.insert(number:, message:)

        expect(subject.get_matching(number:, message:, within_minutes: 60).all.map(&:id)).to eq [1, 3, 4]
      end

      it "returns matching messages with number" do
        described_class.insert(number:, message: "foo")
        described_class.insert(number: "foo", message: "bar")
        described_class.insert(number:, message: "baz")
        described_class.insert(number: "bar", message: "qux")

        expect(subject.get_matching(number:, within_minutes: 60).all.map(&:id)).to eq [1, 3]
      end
      ...
      it "doesn't include old messages" do
        ...
        expect(subject.get_matching(number:, within_minutes: 60).all.map(&:id)).to eq [4, 5, 6]
      end
