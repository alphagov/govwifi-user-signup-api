describe App do
  describe 'Signing up to GovWifi via the text message service' do
    ...
    it 'sends an SMS containing login details back to the user' do
      post '/user-signup/sms-notification/notify', source_number: from_phone_number, message: 'Go', destination_number: ''
    ...
    context 'with a a phone texting itself' do
      shared_examples "rejecting an SMS" do
        ...
        let(:subject) { post '/user-signup/sms-notification/notify', source_number: from_phone_number, message: 'Go', destination_number: to_phone_number }
